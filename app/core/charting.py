...
class Chart:
    ...
    def line(ax, df, title, x_label, y_label, col, color1="red",
        ...
        data = Chart.apply_limit(data, limit)
    ...
    def histogram(ax, df, title, col, x_label, y_label,
        ...
        values = Chart.apply_limit(df[col], limit)
    ...
    def create_chart(chart_type, df, fig=None, *,
        ...
        if chart_type == "line":
            Chart.line(ax, df, title, x_label, y_label,
                 col=col,
                 color1=color1,
                 marker=marker,
                 markersize=markersize,
            ...
            Chart.bar(ax, df, title, x_label, y_label,
                col=col, color1=color1, grid=grid, limit=limit)

        elif chart_type == "pie":
            Chart.pie(ax, df, title, col=col, limit=limit)

        elif chart_type == "histogram":
            Chart.histogram(ax, df, title, col,
                      x_label, y_label,
